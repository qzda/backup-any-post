...
export function queryElementByXPath(xpath: string, contextNode: Node = document) {
  const element = document.evaluate(xpath, contextNode, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
  return element;
}

export function queryElementByXPathAll(xpath: string, contextNode: Node = document) {
  const elements = document.evaluate(xpath, contextNode, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
  return elements;
}